...
export function ShoppingListsPage() {
  ...
  const createList = handleSubmit(async (values) => {
    setApiError(null);
    try { await shoppingListApi.create(values); reset(); setShowForm(false); await queryClient.invalidateQueries({ queryKey: ["shopping-lists"] }); } catch (error) { setApiError(error instanceof ApiError ? error.message : "Não foi possível criar a lista."); }
  });
