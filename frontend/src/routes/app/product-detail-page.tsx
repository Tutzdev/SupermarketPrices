...
export function ProductDetailPage() {
  ...
  return (
    <>
      ...
      <PageHeading title={product.data.name} description={[product.data.brand, product.data.category, product.data.gtin].filter(Boolean).join(" • ") || "Produto do catálogo Gomo"} />
      <div className="grid gap-6 lg:grid-cols-[0.75fr_1.25fr]">
        <section className="surface p-5"><h2 className="font-bold">Detalhes</h2><dl className="mt-5 space-y-4 text-sm">{[["Marca", product.data.brand], ["Categoria", product.data.category], ["GTIN", product.data.gtin], ["Unidade", product.data.unit], ["Quantidade", product.data.quantity], ["Atualizado em", formatDate(product.data.updatedAt)]].map(([term, value]) => <div key={String(term)} className="flex justify-between gap-5 border-b border-border pb-3"><dt className="text-muted">{term}</dt><dd className="text-right font-semibold">{value ?? "Não informado"}</dd></div>)}</dl>{product.data.description ? <p className="mt-5 text-sm leading-6 text-muted">{product.data.description}</p> : null}</section>
