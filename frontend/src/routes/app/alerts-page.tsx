...
export function AlertsPage() {
  ...
  const products = useQuery({ queryKey: ["products", "alerts"], queryFn: () => catalogApi.products({ size: 100 }) });
  ...
  const productNames = new Map(products.data?.content.map((product) => [product.id, product.name]));
